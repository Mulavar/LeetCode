...
public class P51NQueens {
    public List<List<String>> solveNQueens(int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            backtrack(result, curAns, list, visitedBoxes, n, st, 0);
            for (int j=st, k=0;j<n&&k<n;j++,k++) {
                visitedBoxes[j][k]--;
            }
            for (int j=st-1, k=1;j>=0&&k<n;j--,k++) {
    ...
    private void backtrack(List<List<String>> result, char[][] curAns, List<Integer> remained, int[][] visitedBoxes, int n, int curRow, int curCol) {
        ...
        for (int i = 0; i < remained.size(); i++) {
            ...
            curAns[nextRow][curCol + 1] = 'Q';
            if (Math.abs(nextRow - curRow) > 1&&visitedBoxes[nextRow][curCol+1]==0) {
                for (int j=nextRow, k=curCol+1;j<n&&k<n;j++,k++) {
                    visitedBoxes[j][k]++;
                }
                for (int j=nextRow-1, k=curCol+2;j>=0&&k<n;j--,k++) {
                    visitedBoxes[j][k]++;
                }
